...
class AdminRepository {
  ...
  final Storage _fbstorage = Storage();

  Stream<List<Future<User?>>> getRegisterRequestList(String tag) {
    switch(tag){
      case"Vet":     return _firestore
          .collection('users')
          .where('isApproved', isEqualTo: true)
          .where('role', isEqualTo: 'veterinarian')
          .snapshots()
          .map((event) {
        return event.docs.map((d) => getUserDetails(d['uid'].toString(), 'veterinarian')).toList();
      }
      ); break;
      case"Org":     
        return _firestore
        ...
        .where('role', isEqualTo: 'adoption organization')
          .snapshots()
          .map((event) {
          return event.docs.map((d) => getUserDetails(d['uid'].toString(), 'adoption organization')).toList();
      }
      ); break;
      case'All':
      default:    
        return _firestore
          ...
          return event.docs.map((d) => getUserDetails(d['uid'].toString())).toList();
      }
      );
    }
...
switch(role) {
  case 'veterinarian':
    ...
    _user = Vet(uid: uID, name: fn+' '+ln, photo: photo, phone: phone, email: e, experience: ex, degree: degree, specialty: spc);
    });
    break;
  case 'adoption organization':
    await _firestore
     ...
     String license = user['licenseNumber'];
      ...
      _user = Org(uid: uid, name: n, photo: photo, phone: phone, email: e, description: desc, location: loc, website: website, license: license);
    });
    break;
    }
    return _user;
  ...
  Future<void> updateUid(String oldUid, String newUid, String collection) async {
